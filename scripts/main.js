...
function checkForWin(position) {
    ...
    switch (pos) {
        case "00":
            return checkDiagonalWin(position) || checkHorizontalWin(position) || checkVerticalWin(position);
        case "01":
...
function checkHorizontalWin(position) {
    ...
    let col = position[1];
    if (col == 0) {
        if (playBoard[row][col] == playBoard[row][col+1] && playBoard[row][col] == playBoard[row][col+2]) {
            return true;
        }
    }
    if (col == 1) {
        if (playBoard[row][col] == playBoard[row][col-1] && playBoard[row][col] == playBoard[row][col+1]) {
            return true;
        }
    }
    if (col == 2) {
...
function checkVerticalWin(position) {
    ...
    let col = position[1];
    if (row == 0) {
        if (playBoard[row][col] == playBoard[row+1][col] && playBoard[row][col] == playBoard[row+2][col]) {
            return true;
        }
    }
    if (row == 1) {
        if (playBoard[row][col] == playBoard[row-1][col] && playBoard[row][col] == playBoard[row+1][col]) {
            return true;
        }
    }
    if (row == 2) {
...
function checkDiagonalWin(position) {
    ...
    let col = position[1];
    if (row == 0 && col == 0) {
        if (playBoard[row][col] == playBoard[row+1][col+1] && playBoard[row][col] == playBoard[row+2][col+2]) {
            return true;
        }
    }
    if (row == 0 && col == 2) {
    ...
    if (row == 1 && col == 1) {
        if (playBoard[row][col] == playBoard[row-1][col-1] && playBoard[row][col] == playBoard[row+1][col+1]) {
            return true;
        }
        if (playBoard[row][col] == playBoard[row-1][col+1] && playBoard[row][col] == playBoard[row+1][col-1]) {
            return true;
...
function disableGrid() {
    let buttons = document.getElementById("game-container").querySelectorAll("div#game-container > div");
